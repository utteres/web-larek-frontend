...
- Содержит свойства:
    - email:string Email для заказа.
    - phone:string Номер телефона для заказа.
    - address:string Адрес доставки.
    - payment:PaymentMethod Метод оплаты (тип PaymentMethod).

#### IOrder:
- Расширяет интерфейс IOrderForm.
- Добавляет свойства:
    - items:string[] Массив строк, содержащий идентификаторы заказанных товаров.
...
#### FormErrors:
- Тип представляющий ошибки формы при валидации.
- Используется для валидации формы заказа.

#### IAppState:
- Описывает состояние приложения.
- Содержит свойства:
    - catalog: элимент описывающий продукт.
...
- Конструктор принимает аргументы:
    - data: объект который инициализирует свойства модели.
...
- Model  служит в качестве базового класса , который предоставляет основные методы для работы с моделью данных. ProductItem используется для управления информацией о продукте. 
- Свойства:
...
- **catalog**:ProductItem[] Массив ProductItem, содержащий информацию о продуктах.
...
#### Методы:
- `toggleOrderedProduct(id: string, isIncluded: boolean)`:void  Добавляет или удаляет товар из заказа.
- `clearBasket()`:void Очищает корзину.
- `getProducts()`:ProductItem[] Возвращает продукты из каталога.
- `setCatalog(items: IProduct[])`:void Устанавливает каталог продуктов.
- `getTotal()`:number Вычисляет общую сумму заказа.
- `setPreview(item: ProductItem)`:void Устанавливает предпросматриваемый продукт.
- `setOrderField(field: keyof IOrderForm, value: 'online'|'offline')`:void Устанавливает значения полей заказа.
- `validateOrder()`:boolean Проверяет правильность заполнения формы заказа(валидация).
...
- Метод `getProductItem(id: string)`:Promise<IProduct> Отправляет запрос на получение информации о продукте с указанным идентификатором. Обрабатывает полученные данные, добавляя URL-адрес CDN к изображению продукта.
- Метод `getProductList()`:Promise<IProduct[]> Отправляет запрос на получение списка продуктов. Также обрабатывает полученные данные, добавляя URL-адрес CDN к изображениям продуктов.
- Метод `orderProduct(order: IOrder)`:Promise<IOrderResult> Отправляет запрос на оформление заказа с переданными данными. Возвращает результат заказа.
...
Form выступает в роли основного класса для работы с формами, предоставляя необходимый функционал. Класс Order использует этот функционал для эффективного управления данными, связанными с формой заказа.
...
#### Методы:
- `getPaymentMethod()`:'online' | 'offline' | null Возвращает текущий выбранный метод оплаты.
- `updatePaymentButtons()`:void Обновляет стили кнопок оплаты в зависимости от выбранного метода оплаты.
- `handleNextButtonClick()`:void Обрабатывает нажатие кнопки "Далее". Проверяет, указан ли адрес и выбран ли способ оплаты, и отправляет событие о необходимости открытия формы контактов.
